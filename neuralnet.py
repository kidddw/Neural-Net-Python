...
"""Neural Net learning algorithm
   ...
   if "training" option is turned on, use backpropagation to learn weights and write to file
   if "training" option is turned off, use preexisting weights from file to asses error
...
class Define_Data:
  ...
  def import_data(self,x_fname,y_fname):
    """Import data. X matrix as numpy matrix type, y vector as array type
       Assume y data is presented as integers ranging from 1 to number-of-classes""" 
...
def initialize_theta(arch,epsilon):
  """initialize theta matrices with random numbers between -epsilon and epsilon. list of numpy matrices"""
  tens = []
...
def sigmoid(mat):
  """replace each element with the sigmoid function evaluated using that element's value"""
...
def cost_function(Theta_mat,data,lambd):
  ...
  for l in range(L):
    if l == 0: continue
    Theta = Theta_mat[l-1]
    b_reg.append(np.identity(Theta.shape[1])); b_reg[l-1][0,0]=0.0
  hyp = feed_forward(Theta_mat, data.X_train)[L-1]
#  print np.amax(hyp) , np.amin(hyp)
  J = (1.0/data.m_train)*np.trace( -np.dot(np.transpose(data.Y_train), np.log(hyp)) -  
    np.dot(np.transpose(np.ones(data.Y_train.shape) - data.Y_train), np.log(np.ones(hyp.shape) - hyp)) )
  for l in range(L-1):
    J = J + (0.5*lambd/data.m_train) * ( np.trace( np.transpose(Theta_mat[l])*Theta_mat[l]*b_reg[l] ) )
  return J
...
def feed_forward(Theta_mat, X):
  ...
  L = len(Theta_mat) + 1
  for l in range(L):
    if l == 0:  a.append(X);  z.append([0.0]);  continue
    Theta = Theta_mat[l-1]
    a_vec = add_one(a[l-1])
...
def grad_cost_function(Theta_mat,data,lambd):
  ...
  for ll in range(L):
    if ll == L-1: continue
    l=L-ll-1
    if l == L-1: delta[l] = a[l] - data.Y_train; continue
  ...
  for l in range(L):
    if l == L-1: continue
    Delta[l] = np.transpose(delta[l+1]) * add_one(a[l])  #not sure why has to be np.dot
    Theta = Theta_mat[l]
    b_reg.append(np.identity(Theta.shape[1])); b_reg[l][0,0]=0.0
    Theta_grad.append( (1.0/data.m_train) * Delta[l] + (lambd/data.m_train) * Theta[l] * b_reg[l] ) 
  return Theta_grad
...
def grad_cost_function_fd(Theta_mat,data,lambd):
  ...
  for l in range(L):
    if l == L-1: continue
    Theta = Theta_mat[l]
    numgrad = np.zeros(Theta.shape); perturb = np.zeros(Theta.shape)
...
def unroll(matrices,arch):
  """return numpy matrix type column vector of unrolled matrixes. arch determines the dimensions of each matrix"""
  L=len(arch)
  vec = []
  for l in range(L):
    if l == 0: continue
...
def rollup(vector,arch):
  """return list of numpy matrix type matrices from vector. arch determines the dimensions of each matrix"""
  L=len(arch)
  tens = []
  ind = 0
  for l in range(len(arch)):
...
def main():
  ...
  arch = [mydata.X_train.shape[1]] + import_architecture('arch.dat') + [mydata.Y_train.shape[1]]
  lambda_reg = 1.0

  if run_type == 1: # Train weights from training set data
    open('cost.dat', 'w').close()
    Theta_matrices = initialize_theta(arch,1.2)
    args = (mydata.X_train, mydata.Y_train, lambda_reg, arch)

    # scipy conjugate gradient
    Theta_matrices = rollup(optimize.fmin_cg(cost_function_vec, unroll(Theta_matrices,arch), fprime=grad_cost_function_vec, args=args),arch)

    # home made conjugate gradient
    #Theta_matrices = rollup(cg.nonlinear_cg(cost_function_vec, unroll(Theta_matrices,arch), grad_cost_function_vec, args),arch)
  ...
  if run_type == 2: # Print diagnostic values for cross validation and test sets from prexisting weights
...
#  if run_type == 3: # Print diagnostic curve for regularization parameter lambda
#    for 
